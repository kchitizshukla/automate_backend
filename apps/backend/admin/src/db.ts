// PostgreSQL access layer for the ADMIN module (database: am_admin).
//
...
if (!connectionString) {
  console.error('\n[admin-backend] ADMIN_DATABASE_URL is not set.');
  console.error('Add it to apps/backend/admin/.env, e.g.');
  console.error('  ADMIN_DATABASE_URL=postgresql://postgres:password@localhost:5432/am_admin\n');
  process.exit(1);
}
...
pool.on('error', (err) => console.error('[admin-backend] idle client error:', err.message));
...
export async function assertDbReady(): Promise<void> {
  const { rows } = await pool.query('SELECT current_database() AS db, COUNT(*)::int AS admins FROM admins');
  console.log(`[admin-backend] connected to ${rows[0].db} (${rows[0].admins} admins)`);
}
